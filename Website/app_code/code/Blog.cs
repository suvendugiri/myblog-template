...
public static class Blog
{
    ...
    {
        get { return WebUtility.UrlDecode(HttpContext.Current.Request.QueryString["author"] ?? string.Empty).Trim().ToLowerInvariant(); }
    }
    ...
    public static IEnumerable<Post> GetPosts(int postsPerPage = 0)
    {
        ...
        {
            posts = posts.Where(p => p.Categories.Any(c => string.Equals(c.TrimStart().TrimEnd(), category, StringComparison.OrdinalIgnoreCase)));
        }

        var author = CurrentAuthor;
        if (!string.IsNullOrEmpty(author))
        {
            posts = posts.Where(p => p.Author.Equals(author, StringComparison.OrdinalIgnoreCase));
        }
    ...
    public static string GetAuthorWisePostCount(string authorName)
    {
        int count = 0;

        count = GetVisiblePosts().Where(p => p.Author == authorName).Count();
